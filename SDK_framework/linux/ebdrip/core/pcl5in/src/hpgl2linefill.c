...
Bool hpgl2op_RF(PCL5Context *pcl5_ctxt)
{
  ...
  if ( hpgl2_scan_terminator(pcl5_ctxt, &terminator) ) {
    ...
    if ( hpgl2_scan_terminator(pcl5_ctxt, &terminator) ) {
      pcl5_id_cache_remove(pcl5_ctxt->resource_caches.hpgl2_user, (pcl5_resource_numeric_id)index) ;
...
bail_out:
  ...
  if ( errored ) {
    if ( new_entry != NULL ) {
      pcl5_id_cache_remove(pcl5_ctxt->resource_caches.hpgl2_user,  new_entry->detail.numeric_id);
      new_entry = NULL;
    }
  }
