                        ...
                        FontIdReply **cached_reply)
{
  ...
  if (string_id->buf != NULL) {
    p_link->string = mm_alloc(mm_pcl_pool, string_id->length, MM_ALLOC_CLASS_PCL_CONTEXT) ;

    if (p_link->string == NULL)
      return error_handler(VMERROR) ;

    bzero((char *)p_link->string, string_id->length);
    p_link->length = string_id->length ;
    HqMemCpy(p_link->string, string_id->buf, string_id->length) ;
  }
...
Bool id_link_matches(FontIdCacheLink *p_link, int32 id, pcl5_resource_string_id* string_id, int32 symbol_set)
{
  ...
  if (p_link->string == NULL && string_id->buf == NULL)
    return (p_link->id == id) ;

  if (p_link->length != string_id->length)
    return FALSE ;

  if (HqMemCmp(p_link->string, p_link->length, string_id->buf, string_id->length) != 0)
    return FALSE ;
  ...
  *match_found = FALSE ;
  ...
  if (!SLL_LIST_IS_EMPTY(&(font_id_cache))) {
    ...
    for (;;) {
      ...
      total_id_search_depth++ ;
#endif
      if (id_link_matches(p_link, id, string_id, symbol_set)) {
        *match_found = TRUE;
        break ;
      }
                          ...
                          Bool *font_found)
{
  ...
  *font_found = FALSE ;
  ...
  /* See if we already have this one cached */
  if (! search_id_cache(id, string_id, font_sel_info->symbol_set,
                        &match_found, &cached))
    return FALSE ;
  ...
  if (! match_found) {
    ...
    if (string_id->buf) {
      ...
      param[p_id].value.string = (const char *)string_id->buf ;
      param[p_id].length = string_id->length ;
    } else {
    ...
    /* Cache it */
    add_id_cache_entry(id, string_id, font_sel_info->symbol_set, reply, &cached) ;
  }
...
Bool do_fontselection_by_id(PCL5Context *pcl5_ctxt, FontInfo *font_info, int32 id,
                            pcl5_resource_string_id *string_id, int32 which_font)
{
  Font *font ;
  FontSelInfo *font_sel_info ;
  SelByIdInfo *sel_by_id_info ;
  ...
  if (do_selection) {
    if (! do_pfin_select_by_id(pcl5_ctxt, font_sel_info, font, id, string_id, &found))
      return FALSE ;
...
Bool set_font_string_id(PCL5Context *pcl5_ctxt, pcl5_resource_string_id *string_id)
{
  FontMgtInfo *font_management_info ;

  HQASSERT(pcl5_ctxt != NULL, "set_font_string_id: NULL pcl5_ctxt") ;
  HQASSERT(string_id != NULL, "set_font_string_id: NULL string id pointer");
  ...
  if ((font_management_info->string_id.buf = mm_alloc(mm_pcl_pool, string_id->length,
                                                      MM_ALLOC_CLASS_PCL_CONTEXT)) == NULL)
    return FALSE ;

  font_management_info->string_id.length = string_id->length ;
  HqMemCpy(font_management_info->string_id.buf, string_id->buf, string_id->length) ;

  return TRUE ;
}

Bool associate_font_string_id(PCL5Context *pcl5_ctxt, pcl5_resource_string_id *string_id)
{
  FontMgtInfo *font_management_info ;
  ...
  */
  ...
  if (font_management_info->string_id.buf == NULL) {
    ...
    if (! pcl5_string_id_cache_insert_aliased_font(pcl5_ctxt->resource_caches.aliased_string_font,
                                                   new_font, &font_management_info->string_id))
      return FALSE ;
  }
...
void find_font_string_alias(PCL5Context *pcl5_ctxt,
                            pcl5_resource_string_id *string_id,
                            pcl5_resource_string_id *aliased_string_id,
                            uint16 *id)
{
  pcl5_font *font ;

  *aliased_string_id = *string_id;
  *id = 0 ;
  ...
  if (font != NULL) {
    ...
    if (font->alias != NULL) {
      ...
      if (font->detail.string_id.buf != NULL) {
        *aliased_string_id = font->detail.string_id;
      } else {
        aliased_string_id->buf = NULL ;
        aliased_string_id->length = 0 ;
...
void find_font_numeric_alias(PCL5Context *pcl5_ctxt, uint16 alias_id,
                             pcl5_resource_string_id *aliased_string_id, uint16 *id)
{
  pcl5_font *font ;
...
Bool select_font_string_id_as_primary(PCL5Context *pcl5_ctxt, pcl5_resource_string_id *string_id)
{
  pcl5_resource_string_id aliased_string_id;
  uint16 aliased_id ;

  find_font_string_alias(pcl5_ctxt, string_id, &aliased_string_id, &aliased_id) ;

  do_fontselection_by_id(pcl5_ctxt, pcl5_get_font_info(pcl5_ctxt), aliased_id,
                         &aliased_string_id, PRIMARY) ;
  return TRUE ;
}

Bool select_font_string_id_as_secondary(PCL5Context *pcl5_ctxt, pcl5_resource_string_id *string_id)
{
  pcl5_resource_string_id aliased_string_id;
  uint16 aliased_id ;
  ...
  */
  ...
  if (font_management_info->string_id.buf != NULL) {
    pcl5_string_id_cache_remove(pcl5_ctxt->resource_caches.aliased_string_font,
                                &font_management_info->string_id) ;
  } else {
    pcl5_resource_numeric_id font_numeric_id = (pcl5_resource_numeric_id)font_management_info->font_id ;
