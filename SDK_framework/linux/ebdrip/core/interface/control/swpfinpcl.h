...
\code
  ...
  ===================
    ...
    Perform a load or store operation on soft fonts or user-defined symbol sets.

    > [0] PCL_MISCOP_RESOURCE
      [1] int     operation   load or store
      [2] int     type        soft fonts or user-defined symbol sets.
...
enum {
  ...
  PCL_MISCOP_RESOURCE          /* Load and store permanent soft fonts and symbol sets */
} ;

/** \brief Operation codes for PCL_MISCOP_RESOURCE */
enum {
  PCL_RESOURCE_OP_LOAD = 0,
  PCL_RESOURCE_OP_STORE
};

/** \brief Resource types for PCL_MISCOP_RESOURCE */
enum {
  PCL_RESOURCE_TYPE_FONT = 0,
  PCL_RESOURCE_TYPE_SS
};
