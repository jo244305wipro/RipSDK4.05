...
struct PCL_RESOURCE_API_20140822 *pcl5permres_api = NULL;

static Bool pcl5_permres_api_init(void)
{
  void *api;
  sw_rdr_result rdr_res;

  HQASSERT(pcl5permres_api == NULL,
           "pcl5_permres_api_init: API pointer not NULL");
  ...
  PCL5Context**  pp_pcl5_ctxt)
{
  ...
  /* Save any permanent resources */
  pcl5_id_cache_store_permanent_patterns(p_pcl5_ctxt->resource_caches.user);
  pcl5_id_cache_store_permanent_macros(&p_pcl5_ctxt->resource_caches);
  pcl5_fontss_store(p_pcl5_ctxt);

  /* Remove all user defined symbol sets and soft font */
  do_symset_control(p_pcl5_ctxt, SYMSETCTRL_DELETE_ALL);
  do_font_control(p_pcl5_ctxt, FONTCTRL_DELETE_ALL);
