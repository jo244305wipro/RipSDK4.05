...
Bool pcl5op_left_paren_X(PCL5Context *pcl5_ctxt, int32 explicit_sign, PCL5Numeric value)
{
  ...
  find_font_numeric_alias(pcl5_ctxt, (uint16)id, &aliased_string_id, &aliased_id) ;
...
/* Send font/ss permanent resource operation command to PFIN module */
static void do_fontss_operation(
  PCL5Context *pcl5_ctxt,
  int op,
  int type)
{
  static sw_datum resource_cmd[] = {
    SW_DATUM_ARRAY(resource_cmd + 1, 3),
    SW_DATUM_INTEGER(PCL_MISCOP_RESOURCE),
    SW_DATUM_INTEGER(0),    /* resource operation */
    SW_DATUM_INTEGER(0)     /* resource type */
  };
  /* Indices into resource_cmd array */
  enum {
    p_array = 0,
    p_reason,
    p_op,
  ...
  sw_datum *pfin_params;
  sw_pfin_result pfin_rc;

  HQASSERT(pcl5_ctxt != NULL,
           "pcl5_fontss_store: NULL pcl5_ctxt pointer") ;

  resource_cmd[p_op].value.integer = op;
  resource_cmd[p_type].value.integer = type;
  pfin_params = resource_cmd;
  pfin_rc = pfin_miscop(pcl5_ctxt->print_state->ufst, &pfin_params);
  HQTRACE(pfin_rc != SW_PFIN_SUCCESS, ("Failed permanent font/ss operation\n"));
}

/* Store permanent user fonts and user defined symbol sets */
void pcl5_fontss_store(
  PCL5Context *pcl5_ctxt)
{
  do_fontss_operation(pcl5_ctxt, PCL_RESOURCE_OP_STORE, PCL_RESOURCE_TYPE_SS);
  do_fontss_operation(pcl5_ctxt, PCL_RESOURCE_OP_STORE, PCL_RESOURCE_TYPE_FONT);
}

/* Load permanent user fonts and user defined symbol sets */
void pcl5_fontss_load(
  PCL5Context *pcl5_ctxt)
{
  do_fontss_operation(pcl5_ctxt, PCL_RESOURCE_OP_LOAD, PCL_RESOURCE_TYPE_SS);
  do_fontss_operation(pcl5_ctxt, PCL_RESOURCE_OP_LOAD, PCL_RESOURCE_TYPE_FONT);
}
