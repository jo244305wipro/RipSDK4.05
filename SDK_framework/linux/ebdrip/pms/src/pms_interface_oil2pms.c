...
int PMS_CheckinBand(PMS_TyBandPacket *ThisBand)
{
  ...
  if(ThisBand == NULL) /* last band */
  {
    ...
    if((g_tSystemInfo.nOILconfig & 0x100) != 0)
 ...
 */
int PMS_CancelPrintedPage(PMS_TyPage *pstPageToDelete)
{
  unsigned int i, j;
  ...
  currently has an issue with clearing part complete pages */
/*
  if (g_tSystemInfo.uUseRIPAhead)
  {
    PMS_IncrementSemaphore(g_semPageComplete);
                     ...
                     PMS_TyBandInfo *ptPMSBandInfo)
{
  ...
  */
  if ( guRIPBandHeight > 0 ) {
    ...
    ptPMSBandInfo->LinesPerBand = PMS_BAND_HEIGHT;
  }
...
PMS_API_FNS PMS_InitAPI()
{
  ...
  apfnRip_IF_KcCalls[EPMS_FN_CancelPrintedPage] = PMS_CancelPrintedPage;
